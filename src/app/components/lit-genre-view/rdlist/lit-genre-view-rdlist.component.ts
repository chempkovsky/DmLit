...
import { ILitGenreViewEdlg } from './../edlg/lit-genre-view-edlg.interface';
...
@Component({
  selector: 'app-lit-genre-view-rdlist',
  templateUrl: './lit-genre-view-rdlist.component.html',
  styleUrls: ['./lit-genre-view-rdlist.component.css']
})
export class LitGenreViewRdlistComponent implements OnInit, IEventEmitterPub, IItemHeightData {
    ...
    constructor(protected route: ActivatedRoute, protected router: Router, protected  frmRootSrv: LitGenreViewService, protected appGlblSettings: AppGlblSettingsService, public dialog: MatDialog, protected cd: ChangeDetectorRef) {
    }
    permMask: number = 0;
    ngOnInit() {
        this.permMask = this.appGlblSettings.getViewModelMask('LitGenreView');
        ...
        if((msk & 1) === 1) {
            this.rowMenuItemsData.push(
             {id: '1', caption: 'Navigate to: LitManuscriptView : Genre', iconName: 'arrow_forward', iconColor: 'primary', enabled: true, 
                data: {
    ...
    onAdd(sender: LitGenreViewSformComponent) {
        let locdata: ILitGenreViewEdlg = {
            title: 'Add item',
        ...
        let w: string = this.appGlblSettings.getDialogWidth('LitGenreView');
        let mw: string = this.appGlblSettings.getDialogMaxWidth('LitGenreView');
        let dialogRef = this.dialog.open(LitGenreViewEdlgComponent, {
              data: locdata,
              maxWidth: mw,
              width: w,
            });
    ...
    onDelete(e: ILitGenreView , sender: LitGenreViewSformComponent ) {
        let locdata: ILitGenreViewEdlg = {
            title: 'Delete item',
    ...
    applyFilter(frm: LitGenreViewSformComponent) {
        ...
        if (act === 'cancel') {
            ...
            let pkpGenreId: any = paramMap.get('genreId');
    ...
    onNavError() {
        ...
        for (var i = len-1; i > -1; i--) {
            if (!(typeof url[i].path === 'undefined')) {
                if (!(url[i].path === null)) {
                    if ('LitGenreView'.toLowerCase() === url[i].path.toLowerCase()) {
                        shft++;
                        break;
                    }
                }

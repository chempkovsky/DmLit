...
import { ILitDialectViewEdlg } from './../edlg/lit-dialect-view-edlg.interface';
...
@Component({
  selector: 'app-lit-dialect-view-rdlist',
  templateUrl: './lit-dialect-view-rdlist.component.html',
  styleUrls: ['./lit-dialect-view-rdlist.component.css']
})
export class LitDialectViewRdlistComponent implements OnInit, IEventEmitterPub, IItemHeightData {
    ...
    constructor(protected route: ActivatedRoute, protected router: Router, protected  frmRootSrv: LitDialectViewService, protected appGlblSettings: AppGlblSettingsService, public dialog: MatDialog, protected cd: ChangeDetectorRef) {
    }
    permMask: number = 0;
    ngOnInit() {
        this.permMask = this.appGlblSettings.getViewModelMask('LitDialectView');
        ...
        if(paramMap.has('depth')) {
            ...
            if (depth > 1) {
                ...
                let hf: Array<IWebServiceFilterRslt> = [];
                    ...
                    else if ('Language'.toLowerCase() === nvgNm) {
                        if ( (4 + 2) > len ) {
                            this.onNavError();
                            return;
                        }
        ...
        if((msk & 1) === 1) {
            this.rowMenuItemsData.push(
             {id: '1', caption: 'Navigate to: LitManuscriptView : Dialect', iconName: 'arrow_forward', iconColor: 'primary', enabled: true, 
                data: {
    ...
    onAdd(sender: LitDialectViewSformComponent) {
        let locdata: ILitDialectViewEdlg = {
            title: 'Add item',
        ...
        let dialogRef = this.dialog.open(LitDialectViewEdlgComponent, {
              data: locdata,
              maxWidth: mw,
              width: w,
            });
    ...
    onDelete(e: ILitDialectView , sender: LitDialectViewSformComponent ) {
        let locdata: ILitDialectViewEdlg = {
            title: 'Delete item',
    ...
    applyFilter(frm: LitDialectViewSformComponent) {
        ...
        if (act === 'cancel') {
            if(!paramMap.has('dialectId')) {
                return;
            }
            let pkpDialectId: any = paramMap.get('dialectId');
            this.frmRootSrv.getone(pkpDialectId )
                .subscribe(
                   (data: ILitDialectView ) => { // success path
                        ...
                        let i: number = frm.dataSource.findIndex(e => {
                            return  (e.dialectId === data.dialectId)                        });
                        if(i < 0) {
    ...
    onNavError() {
        ...
        for (var i = len-1; i > -1; i--) {
            if (!(typeof url[i].path === 'undefined')) {
                if (!(url[i].path === null)) {
                    if ('LitDialectView'.toLowerCase() === url[i].path.toLowerCase()) {
                        shft++;
                        break;
                    }
                }

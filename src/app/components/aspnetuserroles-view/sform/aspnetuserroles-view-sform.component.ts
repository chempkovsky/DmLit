...
export class AspnetuserrolesViewSformComponent implements OnInit, AfterViewInit, IEventEmitterPub, IItemHeightData {
    ...
    displayedColumns:  Array<string> = ['selectAction', 'userId', 'roleId', 'uUserName', 'rName', 'menuAction'];
    @Input('caption') caption: string = 'aspnetuserrolesView';
    ...
    filterDefs: Array<IWebServiceFilterDef> = [
            {fltrName: 'userId', fltrCaption: 'User Id',  fltrDataType: 'string', fltrMaxLen: 128, fltrMin: null, fltrMax: null }
           ,  {fltrName: 'roleId', fltrCaption: 'Role Id',  fltrDataType: 'string', fltrMaxLen: 128, fltrMin: null, fltrMax: null }
           ,  {fltrName: 'uUserName', fltrCaption: 'User Name',  fltrDataType: 'string', fltrMaxLen: 256, fltrMin: null, fltrMax: null }
           ,  {fltrName: 'rName', fltrCaption: 'Role Name',  fltrDataType: 'string', fltrMaxLen: 128, fltrMin: null, fltrMax: null }
        ];
    ...
    @Output('apply-filter') applyFilter: EventEmitter<AspnetuserrolesViewSformComponent> = new EventEmitter<AspnetuserrolesViewSformComponent>();


    constructor(private  frmRootSrv: AspnetuserrolesViewService, protected appGlblSettings: AppGlblSettingsService, protected dialog: MatDialog, private cd: ChangeDetectorRef) {
    }
    ...
    onHiddenFilter() {
        ...
        isDsbl = isDsbl && this.hiddenFilter.some(v => v.fltrName === 'userId');
        if(!isDsbl) {
            lcFltrDfs.push({fltrName: 'uUserName', fltrCaption: 'User Name',  fltrDataType: 'string', fltrMaxLen: 256, fltrMin: null, fltrMax: null });

        }
        isDsbl = true;
        isDsbl = isDsbl && this.hiddenFilter.some(v => v.fltrName === 'roleId');
        if(!isDsbl) {
            lcFltrDfs.push({fltrName: 'rName', fltrCaption: 'Role Name',  fltrDataType: 'string', fltrMaxLen: 128, fltrMin: null, fltrMax: null });
    ...
    onFilter() {
        let flt: IaspnetuserrolesViewFilter = { page: this.currentPageIndex, pagesize: this.currentPageSize };
        ...
        this.frmRootSrv.getwithfilter(flt)
            .subscribe((v: IaspnetuserrolesViewPage) =>{
                this.inQuery = false;
                let pl: number = 0;
    ...
    onSelectRow(e: IaspnetuserrolesView) {

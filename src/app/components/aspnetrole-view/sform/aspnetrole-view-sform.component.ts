...
export class AspnetroleViewSformComponent implements OnInit, AfterViewInit, IEventEmitterPub, IItemHeightData {
    ...
    displayedColumns:  Array<string> = ['selectAction', 'id', 'name', 'menuAction'];
    @Input('caption') caption: string = 'aspnetroleView';
    ...
    filterDefs: Array<IWebServiceFilterDef> = [
            {fltrName: 'id', fltrCaption: 'Role Id',  fltrDataType: 'string', fltrMaxLen: 128, fltrMin: null, fltrMax: null }
           ,  {fltrName: 'name', fltrCaption: 'Role Name',  fltrDataType: 'string', fltrMaxLen: 128, fltrMin: null, fltrMax: null }
        ];
    ...
    @Output('apply-filter') applyFilter: EventEmitter<AspnetroleViewSformComponent> = new EventEmitter<AspnetroleViewSformComponent>();


    constructor(private  frmRootSrv: AspnetroleViewService, protected appGlblSettings: AppGlblSettingsService, protected dialog: MatDialog, private cd: ChangeDetectorRef) {
    }
    ...
    onFilter() {
        let flt: IaspnetroleViewFilter = { page: this.currentPageIndex, pagesize: this.currentPageSize };
        ...
        this.frmRootSrv.getwithfilter(flt)
            .subscribe((v: IaspnetroleViewPage) =>{
                this.inQuery = false;
                let pl: number = 0;

...
@Component({
  selector: 'app-lit-publisher-view-eform',
  templateUrl: './lit-publisher-view-eform.component.html',
  styleUrls: ['./lit-publisher-view-eform.component.css']
})
export class LitPublisherViewEformComponent implements OnInit, IEventEmitterPub {
    ...
    protected _eformControlModel: ILitPublisherView | null = null;
    @Input('eform-control-model') 
        set eformControlModel (inFormControlModel : ILitPublisherView) {
        ...
        get eformControlModel(): ILitPublisherView {
    ...
    InputToControls() {
        ...
        let val: any = null;
        if(! isInputNull) {
            if(!this.DefCntrlByHddn('publisherId')) { this.DefCntrlByModel('publisherId'); }
            if(!this.DefCntrlByHddn('publisherName')) { this.DefCntrlByModel('publisherName'); }
            ...
            if(!this.DefCntrlByHddn('publisherId') ) { this.mainFormGroup.patchValue({'publisherId': null}); };
            if(!this.DefCntrlByHddn('publisherName') ) { this.mainFormGroup.patchValue({'publisherName': null}); };
    ...
    constructor(private frmSrvLitCountryView: LitCountryViewService, private frmSrvLitPublisherView: LitPublisherViewService, public dialog: MatDialog, protected appGlblSettings: AppGlblSettingsService ) {
        this.appearance = this.appGlblSettings.appearance;
        this.mainFormGroup =  new FormGroup({});
        let locValidators: ValidatorFn[]; 
        locValidators = [ ];
        this.mainFormGroup.addControl('publisherId', new FormControl({ value: null, disabled: true}, locValidators));
        locValidators = [ Validators.required,Validators.maxLength(40),Validators.minLength(3) ];
        this.mainFormGroup.addControl('publisherName', new FormControl({ value: null, disabled: true}, locValidators));
    ...
    OnEformModeChanged(): void {
        if ((this.eformMode === EformMode.AddMode)  || (this.eformMode === EformMode.UpdateMode) ) {
            this.mainFormGroup.controls['publisherId'].enable( {emitEvent: false} );
            this.mainFormGroup.controls['publisherName'].enable( {emitEvent: false} );
...
//
    ...
    cCountryNameSrchClck(): void {
        if ((this.eformMode === EformMode.AddMode)  || (this.eformMode === EformMode.UpdateMode) ) {
            ...
            let w: string = this.appGlblSettings.getDialogWidth('LitPublisherView');
            let mw: string = this.appGlblSettings.getDialogMaxWidth('LitPublisherView');
    ...
    @Output('after-submit') afterSubmit = new EventEmitter<ILitPublisherView>();
    ...
    doSubmit() {
        ...
        if (this.eformMode === EformMode.AddMode) { 
            this.frmSrvLitPublisherView.addone(this.mainFormGroup.getRawValue())
            .subscribe(
                (respdata: ILitPublisherView ) => { // success path
            ...
            this.frmSrvLitPublisherView.updateone(this.mainFormGroup.getRawValue())
            .subscribe(
                (respdata: ILitPublisherView ) => { // success path
                    this.afterSubmit.emit(respdata);
                },
            ...
            this.frmSrvLitPublisherView.deleteone(  this.mainFormGroup.controls['publisherId'].value )
            .subscribe(
                (respdata: ILitPublisherView ) => { // success path
                    this.afterSubmit.emit(respdata);
                },
...
} // the end of the LitPublisherViewEformComponent class body

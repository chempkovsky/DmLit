...
import { ILitPublisherViewEdlg } from './../edlg/lit-publisher-view-edlg.interface';
...
@Component({
  selector: 'app-lit-publisher-view-rdlist',
  templateUrl: './lit-publisher-view-rdlist.component.html',
  styleUrls: ['./lit-publisher-view-rdlist.component.css']
})
export class LitPublisherViewRdlistComponent implements OnInit, IEventEmitterPub, IItemHeightData {
    ...
    constructor(protected route: ActivatedRoute, protected router: Router, protected  frmRootSrv: LitPublisherViewService, protected appGlblSettings: AppGlblSettingsService, public dialog: MatDialog, protected cd: ChangeDetectorRef) {
    }
    permMask: number = 0;
    ngOnInit() {
        this.permMask = this.appGlblSettings.getViewModelMask('LitPublisherView');
        ...
        if((msk & 1) === 1) {
            this.rowMenuItemsData.push(
             {id: '1', caption: 'Navigate to: LitBookView : Publisher', iconName: 'arrow_forward', iconColor: 'primary', enabled: true, 
                data: {
                    view: 'LitBookView',
                    nav: 'Publisher',
                }
    ...
    onAdd(sender: LitPublisherViewSformComponent) {
        let locdata: ILitPublisherViewEdlg = {
            title: 'Add item',
        ...
        let dialogRef = this.dialog.open(LitPublisherViewEdlgComponent, {
              data: locdata,
              maxWidth: mw,
              width: w,
            });
    ...
    onDelete(e: ILitPublisherView , sender: LitPublisherViewSformComponent ) {
        let locdata: ILitPublisherViewEdlg = {
            title: 'Delete item',
    ...
    applyFilter(frm: LitPublisherViewSformComponent) {
        ...
        if (act === 'cancel') {
            ...
            let pkpPublisherId: any = paramMap.get('publisherId');
                ...
                .subscribe(
                   (data: ILitPublisherView ) => { // success path
                        ...
                        let i: number = frm.dataSource.findIndex(e => {
                            return  (e.publisherId === data.publisherId)                        });
                        if(i < 0) {
    ...
    onNavError() {
        ...
        for (var i = len-1; i > -1; i--) {
            if (!(typeof url[i].path === 'undefined')) {
                if (!(url[i].path === null)) {
                    if ('LitPublisherView'.toLowerCase() === url[i].path.toLowerCase()) {
                        shft++;
                        break;
                    }
                }
